...
entity mcu_v1_core_pipe2_tb is
end entity mcu_v1_core_pipe2_tb;

architecture sim of mcu_v1_core_pipe2_tb is
...
begin
    ...
    dut : entity work.mcu_v1_core_pipe2
        generic map (
    ...
    stim : process
        ...
        begin
            ...
            assert output_rdata = slv16(value)
                report "pipe2 FFT output slot " & integer'image(slot)
    ...
    begin
        ...
        assert halted_debug = '1' report "pipe2 FFT did not reach DONE self-loop" severity failure;
        assert illegal_debug = '0' report "pipe2 FFT hit illegal instruction" severity failure;
        assert pc_debug = x"000001A8" report "pipe2 FFT PC should be at DONE" severity failure;
        assert instr_debug = x"E8FFFFFE" report "pipe2 FFT DONE instruction mismatch" severity failure;

        for slot in FFT_EXPECTED_OUTPUT'range loop
            expect_output(slot, FFT_EXPECTED_OUTPUT(slot));
        end loop;

        report "mcu_v1_core_pipe2_tb cycles_to_halt " & integer'image(cycles_to_halt) severity note;
        report "mcu_v1_core_pipe2_tb passed" severity note;
        finish;
    end process;
