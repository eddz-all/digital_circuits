...
entity mcu_8core_core_tb is
end entity mcu_8core_core_tb;

architecture sim of mcu_8core_core_tb is
...
begin
    ...
    stim : process
        ...
        begin
            ...
            assert output_rdata = slv16(value)
                report "output slot " & integer'image(slot)
    ...
    begin
        ...
        for slot in FFT_SIGNAL_INPUT'range loop
            write_input(slot, FFT_SIGNAL_INPUT(slot));
        end loop;
        ...
        for i in 1 to 100 loop
            wait until rising_edge(clk);
            exit when halted_debug = '1' or illegal_debug = '1';
        end loop;

        assert illegal_debug = '0' report "8-core DSP core hit illegal instruction" severity failure;
        assert halted_debug = '1' report "8-core DSP core did not halt" severity failure;

        for slot in FFT_EXPECTED_OUTPUT'range loop
            expect_output(slot, FFT_EXPECTED_OUTPUT(slot));
        end loop;

        report "mcu_8core_core_tb passed" severity note;
        finish;
    end process;
