#!/usr/bin/env python3
"""
Host-side checker for asm/fft8_v5_arm_strict_59.s.

V5 keeps the packed-DSP fixed arithmetic model and adds ARM-real SMLAD/STMIA
instructions to reach the 59 timed-step target exactly.
"""
...
from test_fft8_v4_arm_strict import (  # noqa: E402
    INPUT_BASE,
    IO_BYTES,
    is_input_addr,
    is_output_addr,
    ...
    while True:
        ...
        try:
            if op == "MOV":
                ...
                base_text, reglist_text = split_args(arg_text)
                if not base_text.strip().endswith("!"):
                    raise ValueError("STMIA requires writeback")
                base_reg = parse_reg(base_text.strip()[:-1])
                reglist = parse_reg_list(reglist_text)
                if base_reg in reglist:
                    raise ValueError("STMIA writeback base cannot be in reglist")
                base = regs[base_reg]
                for i, reg in enumerate(reglist):
                    addr = s32(base + 4 * i)
                    if not is_output_addr(addr):
                        raise AssertionError(f"STMIA write to unmapped address {addr}")
                    output_mem[addr] = s16(regs[reg])
                regs[base_reg] = s32(base + 4 * len(reglist))
                external_output_write = True
            elif op == "SHADD16":
                rd_text, ra_text, rb_text = split_args(arg_text)
                ...
                rd_text, ra_text, rb_text, acc_text = split_args(arg_text)
                regs[parse_reg(rd_text)] = smlad(
                    operand_value(ra_text, regs),
                    operand_value(rb_text, regs),
                    operand_value(acc_text, regs),
                )
            elif op == "PKHBT":
                ...
                rd_text, ra_text, rb_text = split_args(arg_text)
                regs[parse_reg(rd_text)] = ssax(operand_value(ra_text, regs), operand_value(rb_text, regs))
            elif op == "SSUB16":
                rd_text, ra_text, rb_text = split_args(arg_text)
                regs[parse_reg(rd_text)] = ssub16(operand_value(ra_text, regs), operand_value(rb_text, regs))
            else:
...
def run_case(
    ...
    *,
    ...
    if interpreted.output != direct:
        ...
        print_flat_vector("V5 output:", interpreted.output)
        print_flat_vector("packed fixed model output:", direct)
        raise AssertionError("V5 assembly does not match packed DSP fixed model")
    if reference == "scalar":
...
def main() -> None:
    program = load_program(ROOT / "asm" / "fft8_v5_arm_strict_59.s")
    ...
    if len(program.instructions) != 78:
        raise AssertionError(f"V5 instruction count {len(program.instructions)} should be 78")
    if result.steps_before_done != 77:
        raise AssertionError(f"V5 steps before DONE {result.steps_before_done} should be 77")
    if result.timed_steps > 59:
        raise AssertionError(f"V5 timed_steps {result.timed_steps} exceeds target 59")
    ...
    print("V5 target timed_steps <= 59 passed.")
