#!/usr/bin/env python3
"""
Host-side checker for asm/fft8_v4_arm_strict.s.

V4 keeps the V3 packed-DSP arithmetic model and adds ARM-real SSAX/SSUB16
instructions to reduce exact -j rotation and sign handling cycles.
"""
...
def ssax(a: int, b: int) -> int:
    return pack(lo16(a) + hi16(b), hi16(a) - lo16(b))


def ssub16(a: int, b: int) -> int:
    return pack(lo16(a) - lo16(b), hi16(a) - hi16(b))
...
def run_case(
    ...
    *,
    ...
    if interpreted.output != direct:
        ...
        print_flat_vector("V4 output:", interpreted.output)
        print_flat_vector("packed fixed model output:", direct)
        raise AssertionError("V4 assembly does not match packed DSP fixed model")
    if reference == "scalar":
...
def main() -> None:
    program = load_program(ROOT / "asm" / "fft8_v4_arm_strict.s")
    ...
    if result.timed_steps > 78:
        raise AssertionError(f"V4 timed_steps {result.timed_steps} exceeds target 78")
    ...
    print("V4 target timed_steps <= 78 passed.")
