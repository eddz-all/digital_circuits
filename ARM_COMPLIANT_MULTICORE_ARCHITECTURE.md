...
该版本给 worker 加入取指/译码寄存，并把 `SMUAD/SMUSD` 拆成多周期 DSP 执行。独立连续 DSP 指令使用局部 pair pipeline，后续常见 `ASR` 可以和第二个 DSP 写回同拍退休。安全相邻的 `MOV/MOV`、`LDR/LDR` 和 `SADD16/SSUB16` 也可以局部双发射同拍退休。若能保持 216 MHz 以上频率，最终 `cnt × period` 仍有竞争力，并且合规性明显强于 butterfly 加速器版本。
