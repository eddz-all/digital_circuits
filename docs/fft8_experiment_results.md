...
本文档集中记录当前 `dsp` 分支下 V1 scalar baseline、V2 packed DSP 加速版和 V3 ARM-safe architecture DSP 优化版的实验数据。当前阶段只做设计与 GHDL 仿真验证，不包含上板或 Vivado timing 数据。
...
```text
V2/V3 对常见边界样例和 1000 组中等幅度随机输入精确匹配 V1 scalar fixed model。
...
mcu_v1_decoder_tb passed @26ns
mcu_v1_core_tb passed @13271ns
```
