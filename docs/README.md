# MCU_4cores_muticycle_arm_a32

This folder is a self-contained four-worker FFT MCU project whose visible ROM
instruction words use the Armv7-A/R AArch32 A32 instruction encoding subset.
...
- Runs four parallel worker cores. Each worker has its own PC, register file,
  32-bit instruction ROM, decoder, A32-decoded ALU/DSP execution, and done/illegal status.
...
- Computes each FFT butterfly through visible A32-encoded ARM/ARM-DSP worker
...
`mcu4_multicycle_core.vhd` contains four worker cores plus one unified generic
data memory. The architectural address used by `LDR/STR` is a 32-bit byte
address. The implemented window is 64 words, byte address `0x00000000..0x000000FC`.
The read path is replicated per worker for timing, while writes update every
replica:

```text
unified dmem word[0..63]
work area A: word[16..23], byte address 0x40..0x5C
work area B: word[32..39], byte address 0x80..0x9C
```
...
`SELFTEST_ROM` for the basic instruction test. These instruction words use the
Armv7-A/R AArch32 A32 standard encoding for the subset listed in
`docs/02_指令定义.md`; they are not generated by RTL encoders. The ROM also emits local predecode fields for
operation, register, immediate and 32-bit byte-address data so the worker does not
...
worker core supports the minimum A32-encoded ARM instruction subset required by the
course brief:
...
The FFT worker program keeps using the shorter A32-encoded ARM/ARM-DSP operation sequence
needed for the FFT:
...
stream remains an A32 standard-encoded ARM/ARM-DSP subset; these are generic micro-architectural
...
asserts that the basic self-test does not write the B/output address region.
The multicycle-level test checks the same A-region result addresses through the
