...
V4 已经保存为 git 回退点：tag `v4-arm-strict-exact`，commit `93db748`，并已推送到 origin/dsp 和远端 tag。
V5 ARM-strict 59 已实现：只新增 ARM 真实存在的 SMLAD/STMIA，达到 59 timed_steps。
...
当前 `dsp` / `origin/dsp` HEAD：commit `f16aba0`，用于修正 V5 push 后的 session memory。
...
V5 已达到 exact 目标 timed_steps <= 59，当前为 59。
...
```text
V1/V2/V3/V4/V5 都不是双核。
V1/V2/V3/V4/V5 都是单核、单周期、单发射 MCU。
tb/mcu_v1_core_tb.vhd 里的 basic_core / fft_core / fast_core / v3_core / v4_core / v5_core 六个实例只是为了 GHDL 同时验证多个程序，不代表最终设计是多核。
```
...
V5 已在 ARM 真实 mnemonic 约束下完成：SMLAD/STMIA + packed DSP。
不要自创融合 mnemonic，不要做 FFT/fixed-twiddle 专用指令，不要硬连 FFT 旁路。
```
...
```text
新增 asm/fft8_v5_arm_strict_59.s，不替换 V1/V2/V3/V4。
...
目标和结果：
  把 V4 的 78 timed_steps 压到 59。
  当前 V5 = 59 timed_steps。
  仍精确匹配 packed DSP fixed model。
  仍不做 FFT 专用硬件旁路、不做自创 mnemonic、不采用 77 cycles risky 近似。
```

下一会话建议执行顺序：

```text
1. 先看本节和 docs/fft8_v5_arm_strict_59_plan.md，再看 0.11 / 0.10 / 0.7 / 0.6。
2. 不要切分支，继续在 dsp 分支。
...
/Users/eddz/work/Digital_Circuits/asm/fft8_v5_arm_strict_59.s
/Users/eddz/work/Digital_Circuits/asm/fft8_v5_arm_strict_59.mem
/Users/eddz/work/Digital_Circuits/asm/fft8_v5_arm_strict_59.lst
/Users/eddz/work/Digital_Circuits/tools/test_fft8_v5_arm_strict_59.py
...
/Users/eddz/work/Digital_Circuits/docs/fft8_v5_arm_strict_59_plan.md
...
```text
V4 已经 commit/tag/push。
commit: 93db748 add v4 arm strict exact fft optimization
tag: v4-arm-strict-exact
branch: dsp
remote: origin/dsp 已包含该提交，远端 tag 已推送。
```

## 0.11 2026-06-03 V5-arm-strict-59 完成记录
...
```text
docs/fft8_v5_arm_strict_59_plan.md
```
...
V5 code commit: 644d857 add v5 arm strict 59 fft optimization
current HEAD: f16aba0 update session memory for v5 push
branch: dsp
remote: origin/dsp 已包含当前 HEAD。
...
```text
名称：V5-arm-strict-59
基线：V4 ARM-strict exact
V4 timed_steps = 78
V5 target timed_steps = 59
按 50 MHz 估算：50 MHz / 59 = 84.75 万次/秒
相对 V1：276 / 59 = 4.68x
预计 DONE PC：0x0134，实际以 assembler 输出为准。
```
...
2. SMLAD + R11=32767 bias，把 W3 的 ASR 后取负 exact 压短 1 条。
3. STMIA R10!, {R0-R5,R11-R12} 两次，替代 8 条 STRD 输出。
```

实现风险：

```text
SMLAD 是四寄存器指令，需要 ALU 第三输入 c = reg_rd3。
STMIA 是 bulk store，需要 regfile bulk read 和 data_mem/work/output bulk write path。
STMIA 不能自动拆 high16；imag 仍必须由 ASR 放到 low16，output_mem 仍只保存 low16。
```
