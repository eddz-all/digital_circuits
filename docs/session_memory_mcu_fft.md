...
V4 尚未实现，但计划已定：V4-arm-strict，只能使用 ARM 真实存在的 mnemonic，继续尝试降低 cnt。
```
...
| 版本 | 说明 | 指令数 | DONE 前执行指令数 | timed_steps / cnt_test | 按 150 MHz 估算速率 | 相对 V1 加速 |
|---|---|---:|---:|---:|---:|---:|
| V1 | scalar baseline | 286 | 285 | 276 | 54.35 万次/秒 | 1.00x |
| V2 | packed DSP | 120 | 119 | 109 | 137.61 万次/秒 | 2.53x |
| V3 | ARM-safe architecture DSP, LDMIA/STRD | 98 | 97 | 88 | 170.45 万次/秒 | 3.14x |
...
```text
V1/V2/V3 都不是双核。
V1/V2/V3 都是单核、单周期、单发射 MCU。
tb/mcu_v1_core_tb.vhd 里的 basic_core / fft_core / fast_core / v3_core 四个实例只是为了 GHDL 同时验证四个程序，不代表最终设计是多核。
```
...
如果还要继续追 PPT 更高档位，需要明确讨论是否接受更专用的 FFT/fixed-twiddle 指令；当前 V3 故意没有做这些。
如果硬约束是“ARM 里真实存在的指令”，则 V4 只能做 ARM-strict 优化，不能自创融合 mnemonic。
当前用户已经确认这个硬约束，所以默认下一步就是 V4-arm-strict。
```
...
1. 先看本节和 0.7 / 0.6。
2. 不要切分支，继续在 dsp 分支。
3. V1/V2/V3 tags 都已保留为回退点。
4. V3 已完成并提交/tag，先跑完整回归确认工作区状态。
5. 若继续做 V4，默认按 ARM-strict 口径：只能使用 ARM 真实存在的 mnemonic。
6. V4 计划见 0.8，先做指令级审计和局部模板搜索，再动 RTL。
```
...
因此 V4 不能走 FFT8 / CMULQ15 / ROTMJ / LDCPLX / STCPLX 这类自定义融合指令路线。
V4 如果继续做，应命名为 V4-arm-strict 或类似口径，只允许 ARM 真实 DSP/访存指令。
...
```text
1. 复乘模板优化：
   候选：SMUADX / SMUSDX / SMLAD / SMLADX / SMLSD / SMLSDX。
...
4. 访存和输出优化：
   候选：LDRD / STRD / LDMIA / STMIA。
   目的：V3 已用 LDMIA / STRD；继续确认是否可用 STMIA 或更好寄存器布局减少输出 ASR。
```
...
所以 V2 的 `109 timed_steps` 实际已经接近用户截图里的 `136.84 万次/秒` 档位；V3 的 `88 timed_steps` 已经进一步超过这一档。若要达到：
...
这意味着下一轮如果还要追更高档位，需要讨论是否接受比 V3 更专用的 FFT/fixed-twiddle 指令。
...
当前 V1/V2/V3 不是双核。
当前是单核、单周期、单发射 MCU。
tb/mcu_v1_core_tb.vhd 里 basic_core / fft_core / fast_core / v3_core 四个实例只是为了 GHDL 同时验证不同程序，不是最终设计中的多核。
```
...
```text
V3 已完成，不要再按旧 fused 指令方案开工。
当前推荐保留 ARM-safe 口径：LDMIA/STRD + packed DSP。
如果用户明确要继续冲更高速度，再讨论 V4 是否引入 FFT/fixed-twiddle 专用指令。
```
...
```text
1. 先确认当前工作区是否已经完成回归。
2. 确认是否需要给 V3 打 tag 或 commit。
3. 如果要继续提速，先明确是保持 ARM-safe，还是允许 V4 专用 FFT 指令。
```
