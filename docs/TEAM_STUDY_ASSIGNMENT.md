...
- 数据存储目前分成 Data RAM Bank A 和 Data RAM Bank B，代码里历史命名是 `buf_a` 和 `buf_b`。
...
- 寄存器堆在哪里，写回信号如何产生。
- `LDR/STR` 如何访问 Data RAM A/B。
- 多周期 DSP 怎么体现，为什么 `SMUAD/SMUSD` 不是简单一拍完成。
- 双发射/配对执行的代码入口在哪里，比如 `WPAIR_MOV_IMM`、`WPAIR_LDR_A`、`WPAIR_STR_B`、`WPAIR_SADD16_SSUB16`。
...
- 全局架构：指令 ROM、四个 worker、寄存器堆、ALU/DSP、Data RAM A/B、board wrapper、ILA。
- `mcu4_multicycle_core.vhd` 如何例化 4 个 worker，`ACTIVE_CORES` 如何决定启用数量。
- Data RAM A/B 在外层如何连接到 worker，为什么代码里还叫 `buf_a` / `buf_b`。
...
- Data RAM A/B 和传统 MCU 的数据存储器是什么关系？
...
| 你们的架构有哪些元件？ | C：画总图，解释 ROM、四核、寄存器堆、ALU/DSP、Data RAM A/B、板级 wrapper | B：解释单个 worker 内部执行数据通路 | A：说明这些元件如何服务 FFT 程序 |
