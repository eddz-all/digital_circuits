...
- 数据存储是一块统一的 32-bit 数据内存；当前程序把 `0x40..0x5C` 当作工作区 A，把 `0x80..0x9C` 当作工作区 B/输出区。
...
- 寄存器堆在哪里，写回信号如何产生。
- `LDR/STR` 如何用 32-bit byte address 访问统一数据内存。
...
- 全局架构：指令 ROM、四个 worker、寄存器堆、ALU/DSP、统一数据内存、board wrapper、ILA。
- `mcu4_multicycle_core.vhd` 如何例化 4 个 worker，`ACTIVE_CORES` 如何决定启用数量。
- 统一数据内存在外层如何连接到 worker，为什么 FFT 程序会把 `0x40..0x5C` 和 `0x80..0x9C` 当作阶段之间的工作区。
...
| 老师问题 | 主答 | 补充 1 | 补充 2 |
| --- | --- | --- | --- |
| 指令格式和编码规则是什么？ | A：说明是 Armv7-A/R AArch32 A32 标准编码子集、支持哪些指令、每类指令用途 | B：说明 decoder 如何取标准字段并生成内部操作 | C：说明指令常量存在哪里，`PROGRAM_ID` 如何选择程序 |
| 你们的架构有哪些元件？ | C：画总图，解释 ROM、四核、寄存器堆、ALU/DSP、统一数据内存、板级 wrapper | B：解释单个 worker 内部执行数据通路 | A：说明这些元件如何服务 FFT 程序 |
