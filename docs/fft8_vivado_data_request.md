...
当前 V1-V5 的软件/GHDL 结果已经完成。现在需要 Vivado 上的真实 timing/resource 数据，回答三个问题：

```text
1. V5 在 50 MHz 下是否稳定过 timing。
2. V5 的关键路径到底在哪里。
3. V5 如果提高到 75 MHz / 100 MHz，是否还有机会过 timing。
```
...
```text
V5 Timing Summary > V5 Worst Timing Path > V5 Utilization
```
...
如果已经有 V1 Timing Summary 截图，可以补：

```text
1. V1 Utilization。
2. V1 Worst Timing Path。
```
...
但 V1 的结果不能替代 V5。V5 因为有 `SMLAD / STMIA / 512-bit bulk path`，critical path 可能不同。
...
```text
1. v5_timing_summary_50m 截图或 .rpt
2. v5_worst_paths_50m 截图或 .rpt
3. v5_utilization_50m 截图或 .rpt
4. .xdc 里的 create_clock 那一行
```

推荐集合：

```text
1. V5 @ 50 MHz Timing Summary
2. V5 @ 50 MHz Worst 10/20 Paths
3. V5 @ 50 MHz Utilization
4. V5 @ 75 MHz Timing Summary
5. V5 @ 100 MHz Timing Summary
6. V5 Methodology / DRC / QoR Suggestions
7. V1 Utilization 和 Worst Path
8. V4 Timing Summary / Utilization / Worst Path
```
